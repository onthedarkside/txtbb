...
$cstatlevel='a';
$LF="\n";
if ($oldcver != $cver || $oldcsubver!=$csubver || $oldcstat!=$cstat || $oldcstatlevel!=$cstatlevel || $oldcsubsubver!=$csubsubver){          /*if the version that is used does not match the stored version number, the stored version number will be updated to show*/
	WriteSettings();
}
...
//	echo "</td></tr></table>";
	if ($do=="news") {
		/*
		 * txtBB modified these lines. forked now.
		 * */
		/*****************************************************************
...
//		}
		...
		if ($ncver>$cver || ($ncver>=$cver && $ncsubver>$csubver) || ($ncver>=$cver && $ncsubver>=$csubver && $ncsubsubver>$csubsubver) ||
			($ncver>=$cver && $ncsubver>=$csubver  && $ncsubsubver>=$csubsubver && ord($ncstatlevel)> ord($cstatlevel))) {
			$suggupd='1';
		}
		if ($ncver==$cver && $ncsubver==$csubver && $ncsubsubver==$csubsubver && $cstat=''){
			$suggupd='0';
		}
		if ($ncver==$cver && $ncsubver==$csubver && ($cstat='rc' || $cstat=='beta' || $cstat=='alpha' || $cstat=='rc')){
			$suggupd='3';
		}
		if ($ncver<$cver ||
		($ncver==$cver && $ncsubver<$csubver) ||
		($ncver==$cver && $ncsubver==$csubver) && $ncsubsubver<$csubsubver ||
		($ncver==$cver && $ncsubver==$csubver && $ncsubsubver==$csubsubver) && ord($ncstatlevel)<ord($cstatlevel)||
		$cstat=='rc' ||
		$cstat=='beta' ||
		$cstat=='alpha'){
...
//		exit;
		if ($sitelogo!=""){
			...
			echo "<td align=\"center\"><b>txtBB</b><br><br>$sitetitle<br>$sitemotto</td>\n";
		}
	}
